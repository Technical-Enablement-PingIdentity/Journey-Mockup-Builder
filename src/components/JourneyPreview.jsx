...
import useDownloadImage from './hooks/useDownloadImg';
import useUrl from './hooks/useUrl';

function JourneyPreview({ journeyOptions }) {
  const mainStyle = {};

  const { downloadPng } = useDownloadImage();
  const { proxyImageUrl } = useUrl();

  if (journeyOptions.pageBackgroundType === 'image') {
    mainStyle.backgroundImage = `url(${proxyImageUrl(
      journeyOptions.pageBackgroundImage
    )})`;
  } else {
  ...
  return (
    <>
          ...
          <main className="site-content d-flex justify-content-center align-items-start">
            ...
            >
              <div className="d-flex justify-content-center mb-2">
                <img
                  className="form-logo"
                  src={proxyImageUrl(journeyOptions.formLogo)}
              ...
              {journeyOptions.showSignInWithApple && (
                <SocialButton
                  iconUrl={proxyImageUrl(
                    'https://img.icons8.com/ios-filled/344/mac-os.png'
                  )}
                  name="Apple"
                  color="rgb(0, 0, 0)"
              ...
              {journeyOptions.showSignInWithFacebook && (
                <SocialButton
                  iconUrl={proxyImageUrl(
                    'https://img.icons8.com/fluency/452/facebook-new.png'
                  )}
                  name="Facebook"
      ...
      <div className="d-flex justify-content-between align-items-center mt-3">
        <div className="me-3" style={{ fontSize: '.75rem' }}>
          **Please note downloaded images may not work properly in Safari.
          Chrome or Firefox is recommended.
        </div>
        <button
          className="btn btn-primary btn-sm"
          onClick={() => downloadPng(document.getElementById('wrapper'))}
        >
          Download PNG
        </button>
      </div>
